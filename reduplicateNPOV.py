...
def checkNpov(w):
    ...
    nw = wordnet_lemmatizer.lemmatize(w)
    if nw in npovdict:
        return True
    else:
        return False
...
def filterChinese(senWl,editWl):
    ...
    for i,ev in enumerate(editWl):
        editWl[i] = RE.sub('',ev)
## comment for looking for parser when running other features. but good and using func
...
def featureGen30(artName,senWl):
    ...
    # wordnet_lemmatizer = WordNetLemmatizer() # for feature 2
...
def biasLexicon(editWl,title):
    wordnet_lemmatizer = WordNetLemmatizer()
    for w in editWl:
        nw = w.lower()
        nw_lemma = wordnet_lemmatizer.lemmatize(nw)

        if nw_lemma in npovDictArticle:
            if title in npovDictArticle[nw_lemma]:
                npovDictArticle[nw_lemma][title] += 1
            else:
                npovDictArticle[nw_lemma][title] = 1
        else:
            npovDictArticle[nw_lemma] = dict()
            npovDictArticle[nw_lemma][title] = 1
...
goodTuples = codecs.open('/Volumes/Seagate Backup Plus Drive/npov_paper_data/npov-edits/goodTuples_testset_Oct9.txt','w','utf-8')
...
# with open('/Volumes/Seagate Backup Plus Drive/npov_paper_data/npov-edits/finalVbiasLexicon_lemma_lower_article2_aug31.json') as t:
#     npovdict = json.load(t)
# npovdict = []
# with open('/Volumes/Seagate Backup Plus Drive/npov_paper_data/bias-lexicon/bias-lexicon.txt') as j:
#     for li in j:
#         li = li.rstrip('\n')
#         npovdict.append(li)
# freqArtDict = {}
                    ...
                    if not brackets0digit(nline[6]) or not brackets0digit(nline[7]):
                        ...
                        if not checkSquarBrac(str1,str2):
                            ...
                            if va == -1:
                                ...
                                # biasLexicon(editWl,nline[0])
                                goodTuples.write(line+'\n')
                                # senWlEditWl.write(str(editWl)+" -----> "+str(senWl)+'\n')
                                # senWlLen.append(len(senWl))
                                # senWlList.append(senWl)
...
# with open('/Volumes/Seagate Backup Plus Drive/npov_paper_data/npov-edits/testdata_features_lst_f32CollaborativeFea.json','w') as fp:
...
# with open('/Volumes/Seagate Backup Plus Drive/npov_paper_data/npov-edits/articleNpovDict_sep11.json','w') as f:
#     json.dump(npovDictArticle,f)
# with open('/Volumes/Seagate Backup Plus Drive/npov_paper_data/npov-edits/testdata_labels_lst.json','w') as fl:
#     json.dump(labels,fl)
...
# with open('/Volumes/Seagate Backup Plus Drive/npov_paper_data/npov-edits/testset_feature_f31_NPOVPAPER_BiasLex_downCase_lemma_2edit_2article.json','w') as u:
#     json.dump(features,u)

# with open('/Volumes/Seagate Backup Plus Drive/npov_paper_data/npov-edits/trainGoodSentenceAfterDataCleanSep30V1.json','w') as t:
#     json.dump(trainOrinSen,t)
